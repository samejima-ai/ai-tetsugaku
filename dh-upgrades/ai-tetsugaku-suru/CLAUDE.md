...
### 1.5 過程が本体であることを見失わない（v2）

本サイトの存在論は「**過程が本体、結晶は過程の区切り**」（SPEC §1.1、REGIME §9）。

- 過程（対話ストリーム）を「結晶のための素材」と扱わない。過程それ自体がサイトの本体
- 未完成・中断・放置を許す。過程に結晶化を急がせない（「気が済んだらやめる」）
- 結晶化の区切り判断は **Master の専権**（論点M）。AI は彫る作業（HTML 化・投稿）のみ担い、自動で区切りを打たない
- 過程の公開は **段階的＋承認ゲート**（論点N）。skill が staging・マスク・整形・commit 準備まで担い、`site/` への公開は Master 承認後。完全自動公開はしない（DONT §2.2）
