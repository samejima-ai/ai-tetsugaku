# INDEX.md — hide-philosophy site

Master (hide) の哲学発信サイト。dialog-harness の利用者プロジェクトとして位置づけ、
dh-upgrades/hide-philosophy/ 配下に仕様一式、リポジトリ root の `site/` 配下に実装を置く。
...
| `SPEC.md` | 何を作るか（コンテンツ・機構・dialogues 機構の Phase 移行） |
| `DONT.md` | 何をやらないか（スコープ外明示） |
| `REGIME.md` | モード / dev_mode / persona / LC |
| `DESIGN.md` | 視覚仕様（色調・タイポ・レイアウト思想） |
...
```
ai-tetsugaku/
├── dh-upgrades/hide-philosophy/   ← 本 SPEC 群（仕様の住処）
